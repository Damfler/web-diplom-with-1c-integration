    ...
    <div class="container px-6 mx-auto grid">
        ...
        <form action="{{ route('admin.slider.add.post') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                ...
                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">Ссылка</span>
                    <input
                            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                            placeholder="Ссылка" name="url"/>
                </label>
                    ...
                    @foreach ($slider as $slide)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3 text-sm">
                                <img src="/{{ $slide->photo }}" alt="{{ $slide->title }}" class="w-20">
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ $slide->title }}
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="{{route('admin.slider.delete', $slide->id)}}" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                       aria-label="Delete">
